...
data_outpath = str(script_dir / 'output_merged/features4dringl4linearsvc_{}.csv')
...
def merge():
    df_all = pd.read_csv(data_all_path, usecols=['id', 'urgency'])
...
def split(df_merged):
    # split with sk learn stratify by urgency
    df_train, df_test = train_test_split(df_merged, test_size=0.35, stratify=df_merged['urgency'])
